...
db.exec(`CREATE TABLE IF NOT EXISTS items(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL,value REAL NOT NULL DEFAULT 0,hands INTEGER NOT NULL DEFAULT 0,updated_at TEXT DEFAULT CURRENT_TIMESTAMP);CREATE TABLE IF NOT EXISTS admins(id INTEGER PRIMARY KEY AUTOINCREMENT,username TEXT UNIQUE,password_hash TEXT NOT NULL);`);
try { db.exec("ALTER TABLE items ADD COLUMN hands INTEGER NOT NULL DEFAULT 0"); } catch(e) {}
...
app.post("/api/items",auth,(q,r)=>{let {name,value}=q.body;if(!name||Number.isNaN(Number(value)))return r.status(400).json({error:"ข้อมูลไม่ถูกต้อง"});const hands=Math.max(0,Math.floor(Number(q.body.hands||0)));r.json({id:db.prepare("INSERT INTO items(name,value,hands) VALUES(?,?,?)").run(name.trim(),Number(value),hands).lastInsertRowid})});
app.put("/api/items/:id",auth,(q,r)=>{let {name,value}=q.body;if(!name||Number.isNaN(Number(value)))return r.status(400).json({error:"ข้อมูลไม่ถูกต้อง"});const hands=Math.max(0,Math.floor(Number(q.body.hands||0)));db.prepare("UPDATE items SET name=?,value=?,hands=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(name.trim(),Number(value),hands,q.params.id);r.json({ok:true})});
